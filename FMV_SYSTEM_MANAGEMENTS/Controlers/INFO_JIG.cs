...
namespace FMV_SYSTEM_MANAGEMENTS.Controlers
{
    public class INFO_JIG
    {
        ...
        public async Task Add(T_INFO_JIG _jig)
        {
            ...
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Jig Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Jig Error : {0}", ex.StackTrace);
            }
        }
        ...
        public async Task AddRange (List<T_INFO_JIG> _lstJig)
        {
            ...
            catch (System.Data.Entity.Validation.DbEntityValidationException ex)
            {
                foreach (var entityValidationErrors in ex.EntityValidationErrors)
                {
                    ...
                    {
                        Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Multi Jig Error Entity: {0}, Property: {1}, Error: {2}", entityValidationErrors.Entry.Entity.GetType().Name, validationError.PropertyName, validationError.ErrorMessage);
                    }
                }
            }
            catch (Exception ex)
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Multi Jig Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Multi Jig Error : {0}", ex.StackTrace);
            }
        }
        ...
        public async Task Update(T_INFO_JIG _jig)
        {
            ...
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Update Jig Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Update Jig Error : {0}", ex.StackTrace);
            }
        }
        ...
        public async Task Delete(int _idjig)
        {
            ...
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Delete Jig Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Delete Jig Error : {0}", ex.StackTrace);
            }
        }
    }
}
