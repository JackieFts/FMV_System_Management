...
namespace FMV_SYSTEM_MANAGEMENT.Controlers
{
    public class COM_STAFF
    {
        ...
        public async Task Add(T_COM_STAFF _staff)
        {
            ...
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Staff Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Add Staff Error : {0}", ex.StackTrace);
            }
        }
        ...
        public async Task Update(T_COM_STAFF _staff)
        {
            ...
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Update Staff Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Update Staff Error : {0}", ex.StackTrace);
            }
        }
        ...
        public async Task Delete(int id)
        {
            ...
            {
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Delete Staff Error : {0}", ex.Message);
                Program.log.LogMsg(Logger.LogLevel.FATAL, "Delete Staff Error : {0}", ex.StackTrace);
            }
        }
    }
}
